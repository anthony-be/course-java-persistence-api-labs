...
public class CustomerDaoImpl implements CustomerDao {
    ...
    @Override
    public Customer findByIdWithFetchGraph(int custId) {
        EntityGraph<?> entityGraph = em.getEntityGraph("Customer.lab11");
        Map<String, Object> props = new HashMap<>();
        props.put("javax.persistence.fetchgraph", entityGraph);
        return em.find(Customer.class, custId, props);
        // Bien que le graph ne demande de charger que quelques attributs, Hibernate est quand même libre de
        // charger d'autres auttributs. Tant que ceux du graph sont chargés, c'est correct.
    }

    @Override
    public Customer findByIdWithLoadGraph(int custId) {
        EntityGraph<?> entityGraph = em.getEntityGraph("Customer.lab11");
        Map<String, Object> props = new HashMap<>();
...
//        query.setParameter("password", pwd);
    ...
    @Override
    public Collection<Customer> findAllWithApi() {
    ...
    @Override
    public Collection<Customer> findAllWithApi(int page, int pageSize) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Customer> cq = cb.createQuery(Customer.class);
        Root<Customer> cust = cq.from(Customer.class);
        cq.select(cust);
        cq.orderBy( cb.asc(cust.get("lastname")));
    ...
    @Override
    public Customer findByUsernameWithApi(String username) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Customer> cq = cb.createQuery(Customer.class);
        Root<Customer> cust = cq.from(Customer.class);
        cq.select(cust);
        cq.where( cb.equal( cust.get("username"), username ) );
    ...
    @Override
    public Collection<Customer> findByCriteriaWithApi(String username, String lastname, String firstname, Date birthDate, Date creationDate) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Customer> cq = cb.createQuery(Customer.class);
        Root<Customer> cust = cq.from(Customer.class);
        cq.select(cust);

        List<Predicate> whereCriteria = new ArrayList<>();

        if(username!=null){
            Predicate pUsername = cb.equal(cust.get(Customer_.username), username);
            whereCriteria.add(pUsername);
        }
        if(lastname!=null){
            Expression<String> upperLastname = cb.upper(cust.get(Customer_.lastname));
            Predicate pLastname = cb.like(upperLastname, "%" + lastname.toUpperCase() + "%");
            whereCriteria.add(pLastname);
        }
        if(firstname!=null){
            Expression<String> upperFirstname = cb.upper(cust.get(Customer_.firstname));
            Predicate pFirstname = cb.like(upperFirstname, "%" + firstname.toUpperCase() + "%");
            whereCriteria.add(pFirstname);
        }

        if(birthDate!=null){
            Predicate pBirthDate = cb.equal(cust.get(Customer_.birthDate), DateUtils.truncate(birthDate, Calendar.DAY_OF_MONTH));
            whereCriteria.add(pBirthDate);
        }

        if(creationDate!=null){
            Predicate pCreatedOn = cb.greaterThan(cust.get(Customer_.createdOn), creationDate);
            whereCriteria.add(pCreatedOn);
        }
        cq.where(whereCriteria.toArray(new Predicate[0]));
