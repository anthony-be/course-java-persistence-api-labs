...
@NamedQueries({
        @NamedQuery(name = "Category.deleteUnusedCategories",
                query = "delete from Category ca where ca.createdOn < :creationDate " +
                        "and NOT EXISTS( select 1 from Product p where p.category = ca)")
})
